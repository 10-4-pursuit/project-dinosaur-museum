...
function calculateTicketPrice(ticketData, ticketInfo) {
  ...
  for (let i = 0; i < ticketInfo.extras.length; i++) {
    ...
    ticketPrice +=
      ticketData.extras[extrasToAdd].priceInCents[ticketInfo.entrantType];
  }
  return ticketPrice;
}
...
function purchaseTickets(ticketData, purchases) {
  ...
  for (let i = 0; i < purchases.length; i++) {
    ...
    if (typeof ticketPrice === "string") {
      ...
      ticketTypeCaps =
        purchases[i].ticketType[0].toUpperCase() +
        purchases[i].ticketType.slice(1);
      extraFormatted = purchases[i].extras
        .map((extra) => extra[0].toUpperCase() + extra.slice(1) + " Access")
        .join(", ");

      if (purchases[i].extras.length === 0) {
        receipt += `${entrantCaps} ${ticketTypeCaps} Admission: $${(
          ticketPrice / 100
        ).toFixed(2)}\n`;
      } else {
        receipt += `${entrantCaps} ${ticketTypeCaps} Admission: $${(
          ticketPrice / 100
        ).toFixed(2)} (${extraFormatted})\n`;
      }
    }
  }
